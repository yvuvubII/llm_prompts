// --- 预设模型列表 (用于搜索提示) ---
const PRESET_MODELS = ["nano banana pro", "nano banana", "豆包", "即梦"];
...
async function init() {
  ...
  if (idParam) {
    ...
    if (searchParam) {
      ...
      // 默认全显示，按ID倒序
      galleryState.filteredData = allData
        .filter((d) => d.visible)
        .sort((a, b) => b.id - a.id);
...
function performSearch(query) {
  ...
  allData.forEach((item) => {
    ...
    // 1. Model 等于 (不区分大小写)
    const hitModel =
      item.model && item.model.some((m) => m.toLowerCase() === lowerQuery);
    // 2. Tag 等于 (不区分大小写)
    const hitTag =
      item.tag && item.tag.some((t) => t.toLowerCase() === lowerQuery);
    // 3. Prompt 包含
    const hitPrompt =
...
function renderGallery() {
  ...
  if (galleryState.currentPage > totalPages && totalPages > 0)
  ...
  if (pageData.length === 0) {
    ...
    listContainer.innerHTML =
      "<div style='text-align:center; color:var(--text-muted); padding:50px;'>暂无内容</div>";
    renderPagination(0);
    // 更新URL
    const url = new URL(window.location);

    if (galleryState.searchQuery)
      url.searchParams.set("search", galleryState.searchQuery);
    window.history.replaceState({}, "", url);
    return;
  } else {
  ...
  pageData.forEach((item) => {
    ...
    el.title = `ID: ${item.id}\n${modelStr}${
      item.prompt ? item.prompt.substring(0, 150) + "..." : ""
    }`;
    ...
    if (item.img && item.img.length > 0) {
      ...
      el.innerHTML =
        "<div style='height:100%; display:flex; align-items:center; justify-content:center; color:var(--text-muted); font-size:0.8em;'>无图片</div>";
    }
...
function renderDetailPage(id) {
  ...
  if (item.model && item.model.length > 0) {
    item.model.forEach((m) => {
      ...
      badge.innerText = m;
      badge.onclick = () => {
        window.location.href = `?search=${encodeURIComponent(m)}`;
      };
      modelArea.appendChild(badge);
    });
  } else {
    modelArea.innerHTML =
      "<span style='color:var(--text-muted); font-size:0.8em'>无模型信息</span>";
  }
  ...
  if (item.tag && item.tag.length > 0) {
    ...
    tagArea.innerHTML =
      "<span style='color:var(--text-muted); font-size:0.8em'>无标签</span>";
  }
  ...
  if (item.img && item.img.length > 0) {
    // 默认显示第一张
    mainImg.src = item.img[detailPageState.currentImgIndex];
    mainImg.style.display = "block";

    // 如果有多张图，生成缩略图
    if (item.img.length > 1) {
      item.img.forEach((src, index) => {
        ...
        // 默认第一张激活
        if (index === detailPageState.currentImgIndex)
          thumb.classList.add("active");

        // 点击缩略图切换大图
        thumb.onclick = () => {
          mainImg.src = thumb.src;
          detailPageState.currentImgIndex = thumb.index;
          // 更新所有缩略图的边框状态
          document
  ...
  function openLightbox(imgList, index) {
    ...
    lightboxState.currentImgList = imgList;
    lightboxState.currentIndex = index;

    const lightbox = document.getElementById("lightbox");
    lightbox.style.display = "flex"; // 显示遮罩
  ...
  function closeLightbox() {
    ...
    thumbs[detailPageState.currentImgIndex].classList.add("active");

    // 更新大图状态
    mainImg.src = thumbs[detailPageState.currentImgIndex].src;
  }
  ...
  function updateLightboxImage() {
    ...
    const list = lightboxState.currentImgList;
    const idx = lightboxState.currentIndex;
    ...
    if (list.length > 1) {
      ...
      nextBtn.style.display = idx < list.length - 1 ? "block" : "none";
    } else {
  ...
  // 点击遮罩背景关闭 (可选)
  document.getElementById("lightbox-container").onclick = (e) => {
  ...
  document.getElementById("lightbox-prev").onclick = (e) => {
    ...
    if (lightboxState.currentIndex > 0) {
      ...
      detailPageState.currentImgIndex = lightboxState.currentIndex;
      updateLightboxImage();
    }
  };
  ...
  document.getElementById("lightbox-next").onclick = (e) => {
    e.stopPropagation();
    if (lightboxState.currentIndex < lightboxState.currentImgList.length - 1) {
      lightboxState.currentIndex++;
      detailPageState.currentImgIndex = lightboxState.currentIndex;
      updateLightboxImage();
    }
  ...
  document.addEventListener("keydown", (e) => {
    if (document.getElementById("lightbox").style.display === "flex") {
      ...
      if (
        e.key === "ArrowRight" &&
        lightboxState.currentIndex < lightboxState.currentImgList.length - 1
      ) {
        lightboxState.currentIndex++;
        detailPageState.currentImgIndex = lightboxState.currentIndex;
        updateLightboxImage();
  ...
  // [修改] 大图点击 -> 打开灯箱 (索引和灯箱外同步)
  mainImg.onclick = () =>
    openLightbox(item.img, detailPageState.currentImgIndex);
  mainImg.style.cursor = "zoom-in"; // 鼠标变为放大镜图标
